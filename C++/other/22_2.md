...
## 理解内存性能
    ...
    cache（缓存） len
        加载和存储-------> 从内存读到寄存器 和 从寄存器写到内存

### 1.加载的性能

    依靠pipeline的能力，依赖加载单元的延迟（迭代）

### 2.存储的性能

    受写入的操作，尤其与加载操作的相互关系，还有一些很细微的关系相关

## 链接

### 简述

    链接是将各种代码和数据片段组合成成一个单一文件的过程，这个单一文件可以被load（copies）到内存执行。
        链接 -->执行于编译时，即源代码翻译成机器码时；
            -->也可执行于加载时，即程序被load到内存并执行的时候；
            -->也可执行在运行时，由应用程序来执行
        在现代系统中，链接叫做链接器（由程序自动执行）
